...
cloudRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid cloud agent message payload", parsed.error.issues);

    const existingProject = parsed.data.projectId
      ? await DeckProjectModel.findById(parsed.data.projectId)
      : null;
    if (parsed.data.projectId && !existingProject) throw ApiError.notFound("Deck project not found");
    if (existingProject) {
    ...
    const deterministicIntent = classifyAgentMessage(parsed.data.message, { hasProject: Boolean(existingProject) });
    const intent = deterministicIntent.confidence < 0.7
      ? await classifyAgentMessageWithLlm(parsed.data.message, Boolean(existingProject), deterministicIntent)
      : deterministicIntent;
    if (intent.intent === "chat") {
      res.json({
        ...
        actions: [
          ...
          ...(existingProject ? [{ type: "edit_deck", label: "Edit current deck" }] : []),
        ],
      });
      return;
    }
    ...
    if (intent.intent === "edit_deck" && existingProject) {
      ...
      const job = await DeckJobModel.create({
        ...
        inputParams: {
          ...
          deckType: artifact?.deckType ?? meta.deckType ?? parsed.data.deckType ?? "general",
          designStyle: artifact?.designStyle ?? meta.designStyle ?? parsed.data.designStyle ?? "modern",
          language: artifact?.language ?? meta.language ?? parsed.data.language ?? "en",
          slideCount: Array.isArray(artifact?.slides) ? artifact.slides.length : parsed.data.slideCount ?? meta.slideCount ?? 10,
          pipeline: "agentic",
          mode: "cloud",
      ...
      await recordUsage(String(existingProject.workspaceId), "deck.job.refine", 1);
      res.status(201).json({
        success: true,
        mode: "cloud",
        type: "job",
    ...
    const project = await DeckProjectModel.create({
      ...
      templateId: null,
      meta: { mode: "cloud", source: "cloud_agent_message", messageIntent: intent },
    });
    const job = await DeckJobModel.create({
...
cloudRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid cloud generation payload", parsed.error.issues);
    const workspaceId = await resolveWorkspaceId(req.auth!.userId, parsed.data.workspaceId);
    const cloud = await effectiveCloudConfig();
    const title = parsed.data.title ?? (parsed.data.prompt.slice(0, 90) || "Untitled cloud deck");
    const project = await DeckProjectModel.create({
      workspaceId,
      ownerId: req.auth!.userId,
      title,
...
cloudRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid cloud edit payload", parsed.error.issues);
    ...
    const job = await DeckJobModel.create({
      ...
      inputParams: {
        ...
        slideCount: Array.isArray(artifact?.slides) ? artifact.slides.length : meta.slideCount ?? 10,
        pipeline: "agentic",
        mode: "cloud",
        cloudProvider: cloud.llmProvider,
        cloudModel: cloud.llmProvider === "mock" ? "mock" : cloud.models[cloud.llmProvider],
...
cloudRouter.post(
  "/jobs/:jobId/continue",
  asyncHandler(async (req, res) => {
    const job = await loadCloudJobForUser(req.params.jobId, req.auth!.userId, "editor");
...
cloudRouter.get(
  "/decks/:projectId",
  asyncHandler(async (req, res) => {
    const project = await loadCloudProjectForUser(req.params.projectId, req.auth!.userId);
    const latestJob = await DeckJobModel.findOne({ projectId: project.id }).sort({ updatedAt: -1 }).lean();
    const meta = isRecord(project.meta) ? project.meta : {};
    res.json({
      success: true,
      mode: "cloud",
...
cloudRouter.get(
  ...
  asyncHandler(async (req, res) => {
    const project = await loadCloudProjectForUser(req.params.projectId, req.auth!.userId);
    const meta = isRecord(project.meta) ? project.meta : {};
    const current = isRecord(meta.deckArtifact) && isRecord(meta.deckArtifact.version) ? meta.deckArtifact.version : null;
    ...
    const versions = jobs
      .map((job) => {
        const resultMeta = isRecord(job.resultMeta) ? job.resultMeta : {};
        const artifact = isRecord(resultMeta.deckArtifact) ? resultMeta.deckArtifact : null;
...
cloudRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid cloud export payload", parsed.error.issues);
    const project = await loadCloudProjectForUser(req.params.projectId, req.auth!.userId, "editor");
    const meta = isRecord(project.meta) ? project.meta : {};
    const artifact = isRecord(meta.deckArtifact) ? meta.deckArtifact : null;
    if (!artifact) throw ApiError.badRequest("Deck has no artifact to export yet");

    const exportFile = parsed.data.format === "pptx"
    ...
    const file = await FileModel.create({
      ...
      storageUrl: `data:${exportFile.mimeType};base64,${exportFile.buffer.toString("base64")}`,
    ...
    await recordUsage(String(project.workspaceId), `deck.export.${parsed.data.format}`, 1);
    res.status(201).json({
      success: true,
      mode: "cloud",
      projectId: project.id,
...
cloudRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!file.storageUrl.startsWith("data:")) throw ApiError.badRequest("Export storage is not directly downloadable");
...
async function resolveWorkspaceId(userId: string, requested?: string): Promise<string> {
  if (requested) {
    const membership = await WorkspaceMemberModel.findOne({ userId, workspaceId: requested });
    if (!membership) throw ApiError.forbidden("Not a member of that workspace");
    return requested;
  }
  const membership = await WorkspaceMemberModel.findOne({ userId }).sort({ createdAt: 1 });
  if (!membership) throw ApiError.badRequest("No workspace available for cloud mode");
  return String(membership.workspaceId);
}

async function loadCloudProjectForUser(projectId: string, userId: string, minRole: "viewer" | "editor" = "viewer") {
  ...
  if (!membership) throw ApiError.forbidden("Not a member of that workspace");
  if (minRole === "editor" && !["owner", "admin", "editor"].includes(membership.role)) {
    throw ApiError.forbidden("Editor role or higher required");
  }
  return project;
}

async function loadCloudJobForUser(jobId: string, userId: string, minRole: "viewer" | "editor" = "viewer") {
...
async function classifyAgentMessageWithLlm(
  ...
  try {
    ...
    const text = await provider.generate(prompt, { temperature: 0, maxTokens: 500 });
    const parsed = extractJsonObject(text);
    const intent = parsed.intent === "create_deck" || parsed.intent === "edit_deck" || parsed.intent === "chat"
      ? parsed.intent
      : fallback.intent;
    return {
      intent,
      confidence: clampConfidence(Number(parsed.confidence ?? 0.75)),
      reason: typeof parsed.reason === "string" ? `llm:${parsed.reason}` : "llm_intent_classifier",
      normalizedMessage: message.replace(/\s+/g, " ").trim(),
      inferredSlideCount: parseOptionalSlideCount(parsed.inferredSlideCount) ?? fallback.inferredSlideCount,
      inferredLanguage: parseOptionalLanguage(parsed.inferredLanguage) ?? fallback.inferredLanguage,
      refinementKind:
        parsed.refinementKind === "design" || parsed.refinementKind === "content" || parsed.refinementKind === "general"
...
function extractJsonObject(text: string): Record<string, unknown> {
  const raw = /```(?:json)?\s*([\s\S]*?)```/i.exec(text)?.[1]?.trim() ?? text.trim();
  try {
...
function buildHtmlExport(title: string, artifact: Record<string, unknown>): { filename: string; mimeType: string; buffer: Buffer } {
  const slides = Array.isArray(artifact.slides) ? artifact.slides.filter(isRecord) : [];
  const body = slides
    .map((slide) => {
      const html =
        (isRecord(slide.preview) && typeof slide.preview.html === "string" ? slide.preview.html : null) ??
        (typeof slide.previewHtml === "string" ? slide.previewHtml : null) ??
        (typeof slide.html === "string" ? slide.html : `<section><h1>${escapeXml(String(slide.title ?? "Slide"))}</h1></section>`);
...
function buildPptxExport(title: string, artifact: Record<string, unknown>): { filename: string; mimeType: string; buffer: Buffer } {
  const slides = (Array.isArray(artifact.slides) ? artifact.slides.filter(isRecord) : []).slice(0, 100);
...
function slideXml(slide: Record<string, unknown>, slideNumber: number): string {
  ...
  const bullets = Array.isArray(slide.bullets) ? slide.bullets.map((b) => escapeXml(String(b))).slice(0, 6) : [];
...
${pptTextShape(3, "Body", bulletText, 900000, 1800000, 10400000, 4200000, 2400, false)}
${pptTextShape(4, "Footer", `YDeck Cloud · ${slideNumber}`, 900000, 6200000, 10400000, 350000, 1200, false)}
</p:spTree></p:cSld><p:clrMapOvr><a:masterClrMapping/></p:clrMapOvr>
</p:sld>`;
}

function pptTextShape(id: number, name: string, text: string, x: number, y: number, cx: number, cy: number, size: number, bold: boolean): string {
  const paragraphs = text.split(/\n+/).filter(Boolean).map((line) => `<a:p><a:r><a:rPr lang="en-US" sz="${size}"${bold ? " b=\"1\"" : ""}/><a:t>${escapeXml(line)}</a:t></a:r></a:p>`).join("");
  return `<p:sp><p:nvSpPr><p:cNvPr id="${id}" name="${escapeXml(name)}"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr><p:spPr><a:xfrm><a:off x="${x}" y="${y}"/><a:ext cx="${cx}" cy="${cy}"/></a:xfrm></p:spPr><p:txBody><a:bodyPr wrap="square"/><a:lstStyle/>${paragraphs || "<a:p/>"}</p:txBody></p:sp>`;
}
...
function corePropsXml(title: string): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${escapeXml(title)}</dc:title><dc:creator>YDeck Cloud</dc:creator><cp:lastModifiedBy>YDeck Cloud</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">${now}</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">${now}</dcterms:modified></cp:coreProperties>`;
}
...
function safeFileName(value: string): string {
  return value.replace(/[^a-z0-9._-]+/gi, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "ydeck";
}
