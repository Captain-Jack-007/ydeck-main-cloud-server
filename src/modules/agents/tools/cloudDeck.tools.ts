...
export function ensureCloudDeckHtml(deck: CloudDeckArtifact): CloudDeckArtifact {
...
export function registerCloudDeckTools(): void {
  ...
  registerTool({
    ...
    execute: async (args, ctx) => {
      ...
      const saved = await saveCloudDeckArtifact(ctx, designed.deck, "design_deck", cloudDesignSummary(designed.report));
      return {
        ...saved,
        content: `${saved.content}\nDesign QA: ${cloudDesignSummary(designed.report)}`,
        data: { ...(isRecord(saved.data) ? saved.data : {}), designReport: designed.report },
      };
    },
  });
  ...
  registerTool({
    ...
    execute: async (args) => {
      ...
      return {
        ok: true,
        content: `Designed slide ${designed.deck.slides[0]?.slideNumber ?? 1}. ${cloudDesignSummary(designed.report)}`,
        data: { slide: designed.deck.slides[0], designReport: designed.report[0] },
      };
    },
  });
  ...
  registerTool({
    ...
    execute: async (args, ctx) => {
      const deck = normalizeDeck(args.deck);
      const saved = await saveCloudDeckArtifact(ctx, deck, "update_deck", args.changeSummary);
      return saved;
    },
  });
}
...
export async function saveCloudDeckArtifact(
  ...
  const existingProject = await DeckProjectModel.findById(ctx.projectId).select("meta").lean();
  const existingMeta = isRecord(existingProject?.meta) ? existingProject.meta : {};
  const previousArtifact = isRecord(existingMeta.deckArtifact) ? existingMeta.deckArtifact : null;
  const previousVersion = isRecord(previousArtifact?.version) ? previousArtifact.version : null;
  const versionNumber = Number(previousVersion?.versionNumber ?? 0) + 1;
  const version = {
    versionId: `v${versionNumber}`,
    versionNumber,
    parentVersionId: typeof previousVersion?.versionId === "string" ? previousVersion.versionId : null,
    reason: changeSummary || source,
    createdAt: new Date().toISOString(),
    jobId: ctx.jobId,
  };
  const artifact = { ...deck, generatedAt: new Date().toISOString(), source, changeSummary, version };
  ...
  ctx.publish?.({
    ...
    payload: { slideCount: deck.slides.length, deckTitle: deck.deckTitle, deckArtifact: artifact },
  });
  return {
    ok: true,
...
function normalizeDeck(deck: CloudDeckArtifact, opts: { forceDesign?: boolean } = {}): CloudDeckArtifact {
  return {
    ...deck,
    slides: deck.slides.map((slide, index) => {
      ...
      return {
        ...
        preview: {
          ...
          slideNumber,
          layoutId: slide.layoutId ?? "html_designed",
          designId: slide.preview?.designId ?? `ydeck.cloud:${escapeAttr(deck.designStyle || "modern")}`,
          html: previewHtml,
        },
      };
    }),
...
function normalizeSlideHtml(
  ...
  const existing = slideSectionFromHtml(slide.html) ?? slideSectionFromHtml(slide.previewHtml) ?? slideSectionFromHtml(slide.preview?.html);
  ...
  return `<section class="ydeck-slide ydeck-${escapeAttr(deck.designStyle)}" style="position:relative;width:1920px;height:1080px;box-sizing:border-box;overflow:hidden;background:#f8fafc;color:#111827;font-family:Inter,Arial,sans-serif;">
  <div class="topbar"><div class="kicker">${kicker}</div><div class="pill">${escapeHtml(labelize(layoutId))}</div></div>
  ${content}
  <footer class="footer"><span>${escapeHtml(deck.deckTitle)}</span><span>${slideNumber} / ${deck.slides.length}</span></footer>
  <style>
...
function renderTitleHero(slide: CloudDeckSlide, bullets: string[], accent: string): string {
  ...
  <main style="position:relative;z-index:2;padding:190px 110px 120px;max-width:1320px">
    ...
    <h1 class="title" style="font-size:86px;margin-top:36px">${escapeHtml(slide.title)}</h1>
    ${slide.subtitle ? `<p class="subtitle">${escapeHtml(slide.subtitle)}</p>` : ""}
    ${bullets.length ? `<div style="display:flex;gap:18px;margin-top:46px;flex-wrap:wrap">${bullets.slice(0, 3).map((b) => `<div class="card small" style="padding:20px 24px;max-width:390px">${escapeHtml(b)}</div>`).join("")}</div>` : ""}
  </main>`;
}

function renderImageSplit(deck: CloudDeckArtifact, slide: CloudDeckSlide, bullets: string[], image: Record<string, unknown>, accent: string): string {
  const src = escapeAttrValue(String(image.storedUrl ?? ""));
  const attribution = typeof image.attributionText === "string" ? image.attributionText : "";
  return `<main style="position:absolute;inset:132px 84px 110px;display:grid;grid-template-columns:0.95fr 1.05fr;gap:54px;align-items:stretch">
    <section style="padding:54px 0 40px">
      ...
      ${slide.subtitle ? `<p class="subtitle">${escapeHtml(slide.subtitle)}</p>` : ""}
      ${bullets.length ? `<div style="display:grid;gap:16px;margin-top:38px">${bullets.slice(0, 3).map((b) => `<div class="small" style="display:flex;gap:14px;align-items:flex-start"><span style="width:12px;height:12px;border-radius:50%;background:${accent};margin-top:9px;flex:0 0 auto"></span><span>${escapeHtml(b)}</span></div>`).join("")}</div>` : ""}
    </section>
    <figure class="card" style="position:relative;margin:0;overflow:hidden;border:0">
      <img src="${src}" alt="${escapeAttrValue(slide.title)}" style="width:100%;height:100%;object-fit:cover;display:block">
      <figcaption style="position:absolute;left:18px;bottom:16px;right:18px;font-size:14px;color:#fff;text-shadow:0 1px 4px rgba(0,0,0,.65)">${escapeHtml(attribution)}</figcaption>
    </figure>
  </main>`;
}

function renderMetricFocus(slide: CloudDeckSlide, bullets: string[], accent: string): string {
  const metrics = metricItems(slide, bullets);
  const bars = metrics.map((m, i) => `<div style="display:flex;align-items:center;gap:18px"><div class="small" style="width:210px;font-weight:750;color:#0f172a">${escapeHtml(m.label)}</div><div style="height:30px;flex:1;background:#e2e8f0;border-radius:999px;overflow:hidden"><div style="height:100%;width:${m.value}%;background:${i % 2 ? "#06b6d4" : accent};border-radius:999px"></div></div><div class="small" style="width:70px;text-align:right;font-weight:800">${m.value}%</div></div>`).join("");
...
function renderTimeline(slide: CloudDeckSlide, bullets: string[], accent: string): string {
  const items = (bullets.length ? bullets : ["Discover", "Design", "Review", "Launch"]).slice(0, 5);
  return `<main style="position:absolute;inset:145px 92px 112px">
    ...
    <div style="display:grid;grid-template-columns:repeat(${items.length},1fr);gap:24px;margin-top:132px">
      ${items.map((item, i) => `<div class="card" style="padding:34px 28px;min-height:220px;position:relative"><div style="position:absolute;top:-68px;left:28px;width:58px;height:58px;border-radius:50%;background:${accent};color:#fff;display:grid;place-items:center;font-size:24px;font-weight:850">${i + 1}</div><div style="font-size:28px;font-weight:850;color:#0f172a;margin-bottom:18px">${escapeHtml(stepLabel(i))}</div><div class="small">${escapeHtml(item)}</div></div>`).join("")}
    </div>
  </main>`;
}

function renderComparison(slide: CloudDeckSlide, bullets: string[], accent: string): string {
...
function renderProblemCards(slide: CloudDeckSlide, bullets: string[], accent: string): string {
  const items = (bullets.length ? bullets : [slide.subtitle ?? slide.title]).slice(0, 4);
  return `<main style="position:absolute;inset:145px 96px 112px">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:24px;margin-top:62px">
      ${items.map((item, i) => `<article class="card" style="padding:34px 28px;min-height:330px;border-top:8px solid ${i % 2 ? "#06b6d4" : accent}">${iconSvg(i % 2 ? "bolt" : "target", i % 2 ? "#06b6d4" : accent)}<div class="small" style="font-size:28px;font-weight:750;color:#0f172a;margin-top:34px">${escapeHtml(item)}</div></article>`).join("")}
    </div>
  </main>`;
}

function renderClosing(slide: CloudDeckSlide, bullets: string[], accent: string): string {
  ...
  <main style="position:absolute;inset:150px 120px 120px;color:#fff;display:grid;place-items:center;text-align:center">
    <section>${iconSvg("spark", accent)}<h1 style="font-size:92px;line-height:1.02;margin:38px auto 30px;font-weight:850;max-width:1240px">${escapeHtml(slide.title)}</h1>
    ${slide.subtitle ? `<p style="font-size:38px;line-height:1.25;color:#cbd5e1;max-width:980px;margin:0 auto">${escapeHtml(slide.subtitle)}</p>` : ""}
    ${bullets[0] ? `<div style="display:inline-block;margin-top:48px;background:${accent};color:#fff;font-size:30px;font-weight:800;padding:22px 34px;border-radius:8px">${escapeHtml(bullets[0])}</div>` : ""}</section>
  </main>`;
}

function renderCardGrid(slide: CloudDeckSlide, bullets: string[], accent: string): string {
  const items = (bullets.length ? bullets : [slide.subtitle ?? slide.title]).slice(0, 6);
  return `<main style="position:absolute;inset:145px 94px 112px;display:grid;grid-template-columns:.9fr 1.1fr;gap:58px;align-items:center">
    <section>${iconSvg("target", accent)}<h1 class="title" style="margin-top:34px">${escapeHtml(slide.title)}</h1>${slide.subtitle ? `<p class="subtitle">${escapeHtml(slide.subtitle)}</p>` : ""}</section>
    <section style="display:grid;grid-template-columns:1fr 1fr;gap:22px">
      ${items.map((item, i) => `<div class="card small" style="padding:28px 26px;min-height:132px;border-left:7px solid ${i % 2 ? "#06b6d4" : accent};font-weight:700;color:#0f172a">${escapeHtml(item)}</div>`).join("")}
    </section>
  </main>`;
}
...
function slidePreviewPayload(deck: CloudDeckArtifact, slide: CloudDeckArtifact["slides"][number]) {
  return {
...
function designQaPayload(deck: CloudDeckArtifact, source: string) {
  const reports = deck.slides
    .map((slide) => {
      const qa = isRecord(slide.visual) && isRecord(slide.visual.designQa) ? slide.visual.designQa : null;
      if (!qa) return null;
      const problems = Array.isArray(qa.problems) ? qa.problems.map((p) => String(p)) : [];
      const fixes = Array.isArray(qa.fixes) ? qa.fixes.map((f) => String(f)) : [];
      const score = typeof qa.previousScore === "number" && qa.previousScore > 0 ? qa.previousScore : undefined;
      return {
        slideNumber: slide.slideNumber,
        slideTitle: slide.title,
        score,
  ...
  return {
    averageScore,
    acceptedSlides: reports.filter((item) => !item.problems.length).length || deck.slides.length,
...
export function designCloudDeckArtifact(
  deck: CloudDeckArtifact,
  opts: { targetScore: number; maxAttempts: number; forceDesign?: boolean; slideCount?: number },
  ...
  working = {
    ...
    slides: working.slides.map((slide, index) => {
      let current: CloudDeckSlide = repairSlideContent(slide, { problems: [], fixes: [], score: 0 }, 0);
      let best: CloudDeckSlide = current;
      let bestQa = scoreSlideDesign(working, best, index, opts.slideCount ?? working.slides.length);
      let attempts = 0;
      const maxAttempts = Math.max(1, Math.min(opts.maxAttempts, 4));

      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        attempts = attempt;
        const html = normalizeSlideHtml(working, current, index, { forceDesign: true });
        const previewHtml = normalizeSlidePreviewHtml(working, current, index, html);
        current = {
          ...current,
          html,
          previewHtml,
          preview: {
            ...
            designId: `ydeck.cloud:${escapeAttr(working.designStyle || "modern")}:${escapeAttr(current.layoutId ?? "html_designed")}`,
            html: previewHtml,
          },
        };
        const qa = scoreSlideDesign(working, current, index, opts.slideCount ?? working.slides.length);
      ...
      report.push({
        ...
        attempts,
        score: bestQa.score,
        accepted: bestQa.score >= opts.targetScore && bestQa.problems.length === 0,
  ...
  return { deck: normalizeDeck(working, { forceDesign: opts.forceDesign }), report };
}

function selectSlideLayout(slide: CloudDeckSlide, index: number): string {
  const type = `${slide.slideType ?? ""} ${slide.title} ${(slide.bullets ?? []).join(" ")}`.toLowerCase();
  if (index === 0 || /\b(title|cover|intro)\b/.test(type)) return "title_hero";
  if (/\b(metric|traction|revenue|growth|kpi|number|stat)\b/.test(type)) return "metric_focus";
  if (/\b(compare|versus|before|after|problem|solution)\b/.test(type)) return "comparison_split";
  if (/\b(timeline|roadmap|process|steps|workflow|journey)\b/.test(type)) return "timeline_process";
  if (/\b(quote|vision|mission|statement)\b/.test(type)) return "quote_statement";
  return "card_grid";
}

function iconSvg(name: "spark" | "chart" | "image" | "bolt" | "target", color: string): string {
  const paths: Record<typeof name, string> = {
    spark: '<path d="M12 2l1.8 6.2L20 10l-6.2 1.8L12 18l-1.8-6.2L4 10l6.2-1.8L12 2z"/><path d="M19 15l.8 2.7L22 18.5l-2.2.8L19 22l-.8-2.7-2.2-.8 2.2-.8L19 15z"/>',
    chart: '<path d="M4 19V5"/><path d="M4 19h16"/><path d="M8 16V9"/><path d="M12 16V6"/><path d="M16 16v-4"/>',
    image: '<rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="8" cy="10" r="1.5"/><path d="M21 16l-5-5-5 5-2-2-4 4"/>',
...
function imageAssetFromSlide(slide: CloudDeckSlide): Record<string, unknown> | null {
  if (!isRecord(slide.visual)) return null;
  const asset = slide.visual.imageAsset;
  return isRecord(asset) && typeof asset.storedUrl === "string" && asset.storedUrl.startsWith("data:image/")
    ? asset
    : null;
}

function metricItems(slide: CloudDeckSlide, bullets: string[]): Array<{ label: string; value: number }> {
  const source = (bullets.length ? bullets : [slide.subtitle, slide.body, slide.title]).filter(Boolean).map(String);
  const values = source.slice(0, 4).map((item, index) => {
    const explicit = Number(item.match(/\b(\d{1,3})\s*%/)?.[1]);
    return {
      label: compactText(item.replace(/\b\d{1,3}\s*%/g, "").trim() || `Signal ${index + 1}`, 42),
      value: Number.isFinite(explicit) ? Math.max(12, Math.min(96, explicit)) : [86, 72, 64, 52][index] ?? 48,
    };
  });
  return values.length ? values : [{ label: "Momentum", value: 82 }, { label: "Adoption", value: 68 }, { label: "Readiness", value: 58 }];
}

function comparisonColumn(title: string, items: string[], color: string): string {
  return `<section class="card" style="padding:38px 38px;min-height:390px;border-top:8px solid ${color}">
    <div style="font-size:34px;font-weight:850;color:#0f172a;margin-bottom:28px">${escapeHtml(title)}</div>
    <div style="display:grid;gap:18px">${items.slice(0, 4).map((item) => `<div class="small" style="display:flex;gap:14px"><span style="width:10px;height:10px;border-radius:50%;background:${color};margin-top:9px;flex:0 0 auto"></span><span>${escapeHtml(item)}</span></div>`).join("")}</div>
  </section>`;
}

function stepLabel(index: number): string {
  return ["Start", "Shift", "Scale", "Proof", "Next"][index] ?? `Step ${index + 1}`;
}
...
function scoreSlideDesign(
  ...
  const chars = [slide.title, slide.subtitle, slide.body, ...bullets].filter(Boolean).join(" ").length;
  ...
  const htmlWithoutStoredImages = html.replace(/data:image\/[a-z0-9+.-]+;base64,[A-Za-z0-9+/=]+/gi, "");
  if (/<script|<iframe|javascript:|https?:\/\//i.test(htmlWithoutStoredImages)) {
    score -= 25;
    problems.push("Unsafe or remote HTML was detected.");
    fixes.push("Remove scripts, iframes, JavaScript URLs, and remote resources.");
  }
  if (titleLength > 96) {
    score -= 10;
  ...
  if (slideCount > 1 && !html.includes(`${slide.slideNumber ?? index + 1} / ${slideCount}`)) {
    score -= 3;
...
function repairSlideContent(
  ...
  return {
    ...
    subtitle: slide.subtitle ? compactText(slide.subtitle, severe ? 130 : 180) : slide.subtitle,
    body: slide.body ? compactText(slide.body, severe ? 220 : 320) : slide.body,
    bullets: compactBullets(slide.bullets ?? [], severe ? 2 : 1),
    layoutId: slide.layoutId && slide.layoutId !== "html_designed" ? slide.layoutId : selectSlideLayout(slide, (slide.slideNumber ?? 1) - 1),
    html: undefined,
...
export function cloudDesignSummary(report: SlideDesignReport[]): string {
  ...
  const avg = Math.round(report.reduce((sum, item) => sum + item.score, 0) / report.length);
...
function compactBullets(bullets: string[], attempt: number): string[] {
  ...
  return bullets.slice(0, maxItems).map((bullet) => compactText(bullet, maxChars)).filter(Boolean);
}
...
function compactText(value: string, maxChars: number): string {
  ...
  return `${clipped.slice(0, boundary > maxChars * 0.55 ? boundary : maxChars - 1).trim()}...`;
}
