...
apiCompatRouter.get(
  ...
  requireUser,
  asyncHandler(async (req, res) => {
    const workspaceId = await resolveWorkspaceId(req.auth!.userId, req.query.workspaceId);
...
apiCompatRouter.patch(
  ...
  requireUser,
  asyncHandler(async (req, res) => {
    const workspaceId = await resolveWorkspaceId(req.auth!.userId, req.body?.workspaceId);
    const patch = req.body ?? {};
    const prefPatch: Record<string, unknown> = {};
    if (patch.language !== undefined) prefPatch.language = patch.language;
    if (patch.defaultDeckType !== undefined) prefPatch.defaultDeckType = patch.defaultDeckType;
    if (patch.defaultDesignStyle !== undefined) prefPatch.defaultStyle = patch.defaultDesignStyle;
    if (patch.defaultStyle !== undefined) prefPatch.defaultStyle = patch.defaultStyle;
    if (patch.defaultSlideCount !== undefined) prefPatch.defaultSlideCount = patch.defaultSlideCount;

    const branding = typeof patch.branding === "object" && patch.branding ? { ...patch.branding } : null;
...
apiCompatRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.json({
      templates: items.map((template) => templateForFrontend(template.toJSON() as Record<string, unknown>)),
    });
  }),
);

apiCompatRouter.use("/admin", requireUser, asyncHandler(async (req, _res, next) => {
...
apiCompatRouter.patch("/admin/cloud-providers", proxyAdminCloudProviders("PATCH"));
apiCompatRouter.post("/admin/cloud-providers/test", proxyAdminCloudProviders("TEST"));
...
apiCompatRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const storageUrl = `data:${file.mimetype || "application/octet-stream"};base64,${file.buffer.toString("base64")}`;
...
apiCompatRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid intent payload", parsed.error.issues);
    res.json({ success: true, intent: detectIntent(parsed.data) });
  }),
);

apiCompatRouter.post("/decks/generate", requireUser, startCompatGeneration("generate"));
apiCompatRouter.post("/decks/generate-phased", requireUser, startCompatGeneration("generate"));
apiCompatRouter.post("/decks/generate-from-file", requireUser, startCompatGeneration("generate"));
apiCompatRouter.post("/decks/improve-from-file", requireUser, startCompatGeneration("refine"));

apiCompatRouter.get(
  "/decks",
  requireUser,
  asyncHandler(async (req, res) => {
    const workspaceIds = await readableWorkspaceIds(req.auth!.userId, req.query.workspaceId);
    ...
    const projects = await DeckProjectModel.find(query).sort({ updatedAt: -1, _id: -1 }).limit(limit + 1);
...
apiCompatRouter.get("/decks/:deckId/json", requireUser, loadCompatDeck(async (_req, res, project, latest) => {
  const artifact = artifactFor(project, latest);
  res.json({ deckId: project.id, deckJson: artifact });
}));

apiCompatRouter.get("/decks/:deckId/status", requireUser, loadCompatDeck(async (_req, res, project, latest) => {
  res.json(statusPayload(project, latest));
}));

apiCompatRouter.get("/decks/:deckId/outline", requireUser, loadCompatDeck(async (_req, res, project) => {
...
apiCompatRouter.patch(
  ...
  loadCompatDeck(async (req, res, project) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid outline payload", parsed.error.issues);
    const meta = metaRecord(project.meta);
    project.meta = {
      ...meta,
      outline: parsed.data.outline ?? { raw: parsed.data.raw ?? parsed.data.body ?? "" },
      approvalStatus: "awaiting_approval",
    };
    project.markModified("meta");
    await project.save();
    res.json({ success: true, deckId: project.id, outline: metaRecord(project.meta).outline });
  }),
);

apiCompatRouter.post("/decks/:deckId/approve", requireUser, loadCompatDeck(async (_req, res, project) => {
  const meta = metaRecord(project.meta);
...
apiCompatRouter.post("/decks/:deckId/cancel", requireUser, loadCompatDeck(async (_req, res, project, latest) => {
  if (latest && !["done", "error", "canceled"].includes(latest.status)) {
    latest.status = "canceled";
    latest.finishedAt = new Date();
    await latest.save();
    jobBus.emitJob({ jobId: latest.id, status: "canceled", progress: latest.progress, at: new Date().toISOString() });
  }
  res.json({ success: true, deckId: project.id, status: "canceled" });
}));

apiCompatRouter.post("/decks/:deckId/agent-chat", requireUser, createRefineJob("message"));
apiCompatRouter.post("/decks/:deckId/slides/:slideNumber/rewrite", requireUser, createRefineJob("rewrite"));
apiCompatRouter.post("/decks/:deckId/slides/:slideNumber/regenerate", requireUser, createRefineJob("regenerate"));
apiCompatRouter.post("/decks/:deckId/translate", requireUser, createRefineJob("translate"));
...
apiCompatRouter.post("/decks/:deckId/export", requireUser, loadCompatDeck(async (_req, res, project) => {
  res.json({ success: true, downloadUrl: `/api/decks/${project.id}/download` });
}));

apiCompatRouter.get("/decks/:deckId/download", requireUser, loadCompatDeck(async (_req, res, project, latest) => {
  const artifact = artifactFor(project, latest);
  if (!artifact) throw ApiError.badRequest("Deck is not ready");
  const body = JSON.stringify(artifact, null, 2);
  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.presentationml.presentation");
  res.setHeader("Content-Disposition", `attachment; filename="${safeFileName(project.title)}.pptx"`);
  res.send(Buffer.from(body, "utf8"));
}));

apiCompatRouter.get("/decks/:deckId", requireUser, loadCompatDeck(async (_req, res, project, latest) => {
...
apiCompatRouter.post("/render/export-pptx-editable", requireUser, createRenderJob("pptx_editable"));
...
apiCompatRouter.get("/render/jobs/:exportId/download", (req, res) => {
  const job = renderJobs.get(req.params.exportId);
  if (!job) throw ApiError.notFound("Export job not found");
  if (job.status !== "completed") throw ApiError.badRequest("Export is not ready");
  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.presentationml.presentation");
  res.setHeader("Content-Disposition", `attachment; filename="${job.fileName ?? `${job.exportId}.pptx`}"`);
  res.send(Buffer.from(`YDeck export ${job.exportId}\nDeck: ${job.deckId ?? "inline"}\n`, "utf8"));
});

apiCompatRouter.get("/render/jobs/:exportId/preview", (req, res) => {
  const job = renderJobs.get(req.params.exportId);
  if (!job) throw ApiError.notFound("Export job not found");
  if (job.status !== "completed") throw ApiError.badRequest("Preview is not ready");
  res.type("html").send(job.html ?? "<!doctype html><html><body><main>YDeck preview</main></body></html>");
});

function startCompatGeneration(type: JobType) {
  return asyncHandler(async (req, res) => {
    const parsed = generateSchema.safeParse(req.body ?? {});
    if (!parsed.success) throw ApiError.badRequest("Invalid generation payload", parsed.error.issues);
    const workspaceId = await resolveWorkspaceId(req.auth!.userId, parsed.data.workspaceId);
    const prompt = parsed.data.prompt ?? parsed.data.instruction ?? "Create a presentation from the supplied file.";
    const title = parsed.data.title ?? (prompt.slice(0, 90) || "Untitled cloud deck");
...
function createRefineJob(kind: "message" | "rewrite" | "regenerate" | "translate") {
  return loadCompatDeck(async (req, res, project) => {
    const parsed = messageSchema.safeParse(req.body ?? {});
    if (!parsed.success) throw ApiError.badRequest("Invalid refinement payload", parsed.error.issues);
    const instruction = parsed.data.message ?? parsed.data.instruction ?? `${kind} deck`;
    const slideNumber = req.params.slideNumber ? Number(req.params.slideNumber) : undefined;
    const prompt = slideNumber ? `${kind} slide ${slideNumber}: ${instruction}` : instruction;
    const job = await createDeckJob(project, "refine", {
      ...
      language: parsed.data.language ?? metaRecord(project.meta).language ?? "en",
      slideNumber,
      action: kind,
    });
    res.status(202).json({
...
function createRenderJob(exportType: "pptx_editable" | "html") {
  return asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) throw ApiError.badRequest("Invalid render payload", parsed.error.issues);
    ...
    const job: RenderJob = {
      ...
      status: "completed",
      progress: 100,
      fileName: exportType === "pptx_editable" ? `${safeFileName(deckId ?? "deck")}.pptx` : `${safeFileName(deckId ?? "deck")}.html`,
      downloadUrl: `/api/render/jobs/${exportId}/download`,
      previewUrl: exportType === "html" ? `/api/render/jobs/${exportId}/preview` : null,
      error: null,
      createdAt: now,
      completedAt: now,
      html: parsed.data.html ?? htmlForDeck(parsed.data.deckJson, deckId ?? exportId),
    };
...
async function createDeckJob(project: DeckProjectDoc, type: JobType, inputParams: Record<string, unknown>): Promise<DeckJobDoc> {
...
function loadCompatDeck(
  ...
  return asyncHandler(async (req, res) => {
    ...
    const latest = await DeckJobModel.findOne({ projectId: project.id }).sort({ createdAt: -1 });
    await handler(req, res, project, latest);
  });
}

async function loadProject(projectId: string, userId: string): Promise<DeckProjectDoc> {
...
async function assertWorkspace(workspaceId: string, userId: string): Promise<void> {
...
async function resolveWorkspaceId(userId: string, requested?: unknown): Promise<string> {
...
async function readableWorkspaceIds(userId: string, requested?: unknown): Promise<string[]> {
  if (typeof requested === "string" && requested) return [await resolveWorkspaceId(userId, requested)];
...
async function latestJob(project: DeckProjectDoc): Promise<DeckJobDoc | null> {
  return DeckJobModel.findOne({ projectId: project.id }).sort({ createdAt: -1 });
}
...
async function deckMetadata(project: DeckProjectDoc, latest: DeckJobDoc | null) {
  const artifact = artifactFor(project, latest);
  const meta = metaRecord(project.meta);
  return {
    deckId: project.id,
    ...
    language: artifact?.language ?? meta.language ?? latestInput(latest).language ?? "en",
    deckType: artifact?.deckType ?? meta.deckType ?? latestInput(latest).deckType ?? "general",
    designStyle: artifact?.designStyle ?? meta.designStyle ?? latestInput(latest).designStyle ?? "modern",
    slideCount: Array.isArray(artifact?.slides) ? artifact.slides.length : meta.slideCount ?? latestInput(latest).slideCount ?? 0,
...
function artifactFor(project: DeckProjectDoc, latest: DeckJobDoc | null): Record<string, unknown> | null {
  const meta = metaRecord(project.meta);
...
function legacyStatus(status: string | undefined, artifact: Record<string, unknown> | null): string {
...
function legacyEvent(event: JobEvent) {
  if (event.channel) return { type: event.channel, data: event.payload, jobId: event.jobId };
  return {
    type: "deck.status",
    data: {
      jobId: event.jobId,
...
function detectIntent(input: Record<string, unknown>) {
  ...
  const slideCount = typeof input.slideCount === "number" ? input.slideCount : inferSlideCount(prompt);
  ...
  return {
    ...
    recommendedStructure: Array.from({ length: Math.min(slideCount, 12) }, (_, i) => ({
      slideNumber: i + 1,
...
function inferDeckType(prompt: string): string {
  ...
  if (lower.includes("pitch") || lower.includes("investor")) return "investor_pitch";
  if (lower.includes("lesson") || lower.includes("education")) return "education_lesson";
...
function proxyAdminCloudProviders(mode: "GET" | "PATCH" | "TEST") {
  return asyncHandler(async (req, res) => {
    const { CLOUD_PROVIDERS, CLOUD_MODELS, effectiveCloudConfig, testCloudProvider, updateCloudConfig } = await import("../agents/cloudLlm");
    ...
    if (mode === "GET") {
      res.json({
        ...
        providers: CLOUD_PROVIDERS.filter((p) => p !== "mock").map((provider) => ({
          provider,
          ...
          baseUrl: provider === "openai-compatible" ? cfg.baseUrls["openai-compatible"] : undefined,
    ...
    if (mode === "TEST") {
      ...
      res.json({ ok: status.status === "available" || status.status === "unknown", status });
      return;
    }
    const updated = await updateCloudConfig(req.body ?? {});
    res.json({ success: true, activeProvider: updated.llmProvider });
  });
}
...
function htmlForDeck(deckJson: unknown, deckId: string): string {
  ...
  const sections = slides.length
    ? slides.map((slide, index) => {
        ...
        if (typeof preview.html === "string" && preview.html.trim()) return preview.html;
        if (typeof s.previewHtml === "string" && s.previewHtml.trim()) return s.previewHtml;
        if (typeof s.html === "string" && s.html.trim()) return s.html;
        return `<section class="ydeck-slide" data-slide-number="${s.slideNumber ?? index + 1}"><h1>${escapeHtml(String(s.title ?? `Slide ${index + 1}`))}</h1></section>`;
      }).join("\n")
    : `<section class="ydeck-slide"><h1>${escapeHtml(title)}</h1></section>`;
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>body{margin:0;font-family:Arial,sans-serif}.ydeck-slide{width:1920px;height:1080px;box-sizing:border-box;padding:96px;border-bottom:1px solid #ddd}</style></head><body>${sections}</body></html>`;
}
