...
This directory holds **slide/deck design templates** — packaged shapes the
agent can use to render presentation artifacts. Each entry is a folder with a
`SKILL.md` plus rendering side files (`example.html`, `assets/`,
`references/`, etc.).

See `../docs/open-design-slide-generation-principles.md` for the YDeck slide
generation rules copied from Open Design.

Only presentation templates belong here. Keep `od.mode: deck` explicit in every
template. Web prototypes, dashboards, landing pages, posters, image/video/audio
templates, reports, research workflows, utilities, audits, and other non-slide
artifacts should live outside this directory.
...
1. Create `design-templates/<my-template>/SKILL.md` with `name`,
   `description`, `triggers`, and explicit `od.mode: deck`.
